...
Shader "Mirza Beig/GPU Fog (URP)"
{
	...
	SubShader
	{
		...
		// Cull Off (was: Cull Back). Keystone edit: the noise-driven vertex
		// displacement in this shader can wind the displaced tessellated
		// triangles "backward" from camera at some view angles, causing
		// every triangle to be backface-culled simultaneously and the
		// whole cloud to vanish on rotation. Two-sided rendering keeps
		// the cloud visible from every angle. If this shader is ever
		// regenerated from its Amplify graph, re-apply this edit.
		Cull Off
		AlphaToMask Off
		...
		Pass
		{
			...
			half4 frag ( VertexOutput IN  ) : SV_Target
			{
				...
				// Keystone edit: hardcoded to 1.0 to bypass scene-depth sampling.
				// The original `saturate(abs(diff / _SurfaceDepthFade))` corner-cases
				// to NaN when the particle's depth exactly matches the depth buffer
				// at the same screen pixel -- which happens intermittently over
				// Timberborn's water surface -- producing whole-cloud alpha collapse.
				// We don't need soft-particles behaviour for ambient mist that sits
				// well above ground. If this is ever regenerated from the Amplify
				// graph, re-apply this edit.
				float SurfaceDepthFade55 = 1.0;
